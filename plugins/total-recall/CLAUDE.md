...
- `no-prune` tag (immortality) — a memory carrying the `no-prune` tag (constant `NO_PRUNE_TAG` in `src/paths.ts`) is (a) excluded from `prune_memories` candidates *before* the retention filter, so it never counts toward `limit` either, and (b) refused by `delete_memory` unless `force=true` is passed (mirrors `store_memory`'s `force` pattern). Use it for decisions that must not decay out of the prune list or be removed by mistake (e.g. an ADR). Tag-only by design: the `decisions` category is NOT auto-protected — not every decision is immortal, immortality is an explicit per-memory opt-in. The guard is orthogonal to authorship, so it blocks even the org author; `force=true` still allows a deliberate teardown. Pinned by `pruneMemories` no-prune exclusion tests in `query.test.ts` and `delete_memory` force-guard tests in `index.test.ts`
